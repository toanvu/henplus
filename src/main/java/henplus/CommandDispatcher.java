...
public class CommandDispatcher implements ReadlineCompleter {
	...
	public CommandDispatcher(SetCommand sc, HenPlus henplus) {
		_henplus = henplus;
	...
	public Iterator completeCommand(String partialCommand, String lastWord){		
		Command cmd = getCommandFrom(partialCommand);		
		if(cmd != null){
			return  cmd.complete(this, partialCommand,lastWord);						
		}
		return null;
	}
	...
	private void informAfterListeners(SQLSession session, String cmd, int result) {
		Iterator it = executionListeners.iterator();
		while (it.hasNext()) {
			ExecutionListener listener = (ExecutionListener) it.next();
		...
		if (c != null) {
			try {
				...
				case Command.SYNTAX_ERROR: {
					...
					if (synopsis != null) {
						_henplus.msg().println("usage: " + synopsis);
					} else {
						_henplus.msg().println("syntax error.");
					}
				}
					break;
				case Command.EXEC_FAILED: {
					 ...
					 */
					if (isInBatch()) {
						_henplus.msg().println("-- failed command: ");
						_henplus.msg().println(givenCommand);
					}
				}
					break;
				default:
...
//		final HenPlus henplus = HenPlus.getInstance();		
		String completeCommandString = _henplus.getPartialLine().trim();
			...
			while (possibleValues.hasNext()) {
				...
				if (text.length() < 1) {
					...
					if (c.requiresValidSession(nextKey)
							&& _henplus.getCurrentSession() == null) {
						continue;
					}
				}
