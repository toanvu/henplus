...
public final class ResultTablePrinter {
	...
	public static int printResult(TableDiffResult result, HenPlus henplus) {
		/*
	...
	private static void appendModified(List rows, ListMap modified) {
		...
		while (iter.hasNext()) {
			...
			henplus.sqlmodel.Column mod = (henplus.sqlmodel.Column) modified
					.get(org);

			ExtendedColumn[] orgView = new ExtendedColumn[8];
			ExtendedColumn[] modView = new ExtendedColumn[8];

			orgView[0] = new ExtendedColumn(STAT_MODIFIED_ORG,
					DESC_META[0].getAlignment());
			modView[0] = new ExtendedColumn(STAT_MODIFIED_NEW,
					DESC_META[0].getAlignment());

			// if this was modified it doesn't matter
			orgView[1] = new ExtendedColumn(org.getPosition(),
					DESC_META[1].getAlignment());
			modView[1] = new ExtendedColumn(mod.getPosition(),
					DESC_META[1].getAlignment());

			// this should not differ
			orgView[2] = new ExtendedColumn(org.getName(),
					DESC_META[2].getAlignment());
			modView[2] = new ExtendedColumn(mod.getName(),
					DESC_META[2].getAlignment());

			String orgType = extractType(org);
			String modType = extractType(mod);
			orgView[3] = new ExtendedColumn(orgType,
					DESC_META[3].getAlignment());
			modView[3] = new ExtendedColumn(modType,
					DESC_META[3].getAlignment());
			...
			String orgDefaultVal = (org.getDefault() != null) ? org
					.getDefault().trim() : null;
			// oracle appends newline to default values for some reason.
			orgView[5] = new ExtendedColumn(orgDefaultVal,
					DESC_META[5].getAlignment());

			String modDefaultVal = (mod.getDefault() != null) ? mod
					.getDefault().trim() : null;
			modView[5] = new ExtendedColumn(modDefaultVal,
					DESC_META[5].getAlignment());
			...
			orgView[6] = new ExtendedColumn(pkDescOrg,
					DESC_META[6].getAlignment());
			modView[6] = new ExtendedColumn(pkDescMod,
					DESC_META[6].getAlignment());
			...
			if (org.isPartOfPk() && !mod.isPartOfPk()) {
				...
				if (org.getPkInfo().getPkName() != null
						&& !org.getPkInfo().getPkName()
								.equals(mod.getPkInfo().getPkName())) {
					markAsChanged(modView[6]);
				}
			}
			...
			orgView[7] = new ExtendedColumn(fkDescOrg,
					DESC_META[7].getAlignment());
			modView[7] = new ExtendedColumn(fkDescMod,
					DESC_META[7].getAlignment());
			// check if one of the cols has to be marked as changed
	...
	private static String getPkDesc(henplus.sqlmodel.Column col) {
		...
		if (col.isPartOfPk()) {
			...
			if (pkInfo.getColumnIndex() == 1) {
				...
				pkDesc = (pkInfo.getPkName() != null) ? pkInfo.getPkName()
						: "*";
				pkDesc = new StringBuffer(pkDesc).append("{")
						.append(pkInfo.getColumnIndex()).append("}").toString();
			}
		}
	...
	private static String getFkDesc(henplus.sqlmodel.Column col) {
		...
		if (fkInfo != null) {
			...
			sb.append(fkInfo.getPkTable()).append("(")
					.append(fkInfo.getPkColumn()).append(")");
			fkDesc = sb.toString();
		}
