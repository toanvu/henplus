...
//	private DataSourceFactory dsFactory;
	...
	public HenPlus(BundleContext context) throws IOException {
		this.context = context;
		//use datasourceFactory if you want to create connection wiht your own url, username and password 
		//also not with datasource from karaf
		//dsFactory = new DataSourceFactory(context,new org.postgresql.Driver(),"postgresql");
		_terminated = false;
		...
		setDefaultPrompt();
		System.out.println("init commands start!");
		String[] defaultParam = { "" };
		initializeCommands(defaultParam);
		System.out.println("init commands done! Press Enter to start Henplus");
	}
	...
	public void initializeCommands(String argv[]) {
		...
		_dispatcher = new CommandDispatcher(_settingStore, this);
		_objectLister = new ListUserObjectsCommand(this);
		_henplusProperties.registerProperty("echo-commands",
				new EchoCommandProperty(_dispatcher, this));

		_dispatcher.register(new HelpCommand(this));

		/*
		 * this one prints as well the initial copyright header.
		 */
		_dispatcher.register(new AboutCommand(_quiet, this));

		_dispatcher.register(new ExitCommand(this));
		_dispatcher.register(new EchoCommand(this));
...
//				dsFactory, this.context));
		_dispatcher.register(new ConnectCommand(this, _sessionManager,this.context));
		
		_dispatcher.register(new StatusCommand(this));

		_dispatcher.register(_objectLister);
		_dispatcher.register(new DescribeCommand(_objectLister, this));

		_dispatcher.register(new TreeCommand(_objectLister, this));

		_dispatcher.register(new SQLCommand(_objectLister, _henplusProperties,
				this));

		_dispatcher.register(new ImportCommand(_objectLister, this));
		// _dispatcher.register(new ExportCommand());
		_dispatcher.register(new DumpCommand(_objectLister, loadCommand, this));

		_dispatcher.register(new ShellCommand(this));
		...
		_dispatcher.register(new SystemInfoCommand(this));
		...
		try {
			...
			for (Iterator it = _dispatcher.getRegisteredCommands(); it
					.hasNext();) {
				Command element = (Command) it.next();
	...
	public void run(String externalCmdLine) {
		String cmdLine = externalCmdLine;
	...
	public OutputDevice msg() {
		// return getInstance().getMessageDevice();
		return this.getMessageDevice();
	}
	...
	public void setCurrentCommand(String selectedCommand){
	 ...
	 */
	public static DataSource getDataSource(String name, BundleContext context) throws InvalidSyntaxException{
		String el = "(osgi.jndi.service.name="+name+")";		
		ServiceReference[] serviceReferences = context.getAllServiceReferences(
				DataSource.class.getCanonicalName(), el);
		if (serviceReferences == null) {
			System.out.println("Datasource "+name+" not found!");
			return null;
		}
		return (DataSource) context.getService(serviceReferences[0]);
	}
}
