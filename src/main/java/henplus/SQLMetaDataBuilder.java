...
public final class SQLMetaDataBuilder {
    ...
    public SQLMetaData getMetaData(SQLSession session, HenPlus _henplus) {
	ResultSet rset = null;
	...
	try {
        ...
        return getMetaData(session, tableList,_henplus);
    }
    
    public SQLMetaData getMetaData(SQLSession session, 
                                   Collection /*<String>*/ tableNames, HenPlus _henplus) {
        return getMetaData( session, tableNames.iterator() , _henplus);
    }

    public SQLMetaData getMetaData(SQLSession session, 
                                   final Iterator /*<String>*/ tableNamesIter, HenPlus _henplus) {
        ...
        try {
            ...
            while (tableNamesIter.hasNext() && !_interrupted) {
                ...
                Table table = buildTable(catalog, meta, tableName, rset,_henplus);
    ...
    public Table getTable(SQLSession session, String tableName, HenPlus _henplus) {
        Table table = null;
        ResultSet rset = null;
        try {
            String catalog = session.getConnection().getCatalog();
            DatabaseMetaData meta = session.getConnection().getMetaData();
            rset = meta.getColumns(catalog, null, tableName, null);
            table = buildTable(catalog, meta, tableName, rset,_henplus);
        }
        catch (Exception e) {
            if (_verbose)
                e.printStackTrace();
    ...
    private Table buildTable(
                             ...
                             String tableName,
                             ResultSet rset, HenPlus _henplus)
        throws SQLException {
        ...
        if (rset != null) {
            ...
            Map fks = getForeignKeys(meta, tableName, _henplus);
    ...
    private Map getForeignKeys(DatabaseMetaData meta, String tabName, HenPlus _henplus)
        throws SQLException {
        Map fks = new HashMap();
        ...
        try {
            ...
            if (_verbose)
            	_henplus.msg().println("Database problem reading meta data: " + e);
        }
