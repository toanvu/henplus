...
public class HelpCommand extends AbstractCommand {
	...
	public HelpCommand(HenPlus henplus){
		_henplus = henplus;
	}
	...
	public int execute(SQLSession session, String cmdstr, String param) {
		 ...
		 */
		if (param == null) {
			Iterator it = _henplus.getDispatcher()
			...
			while (it.hasNext()) {
				...
				if (cmds.length > 1 || firstSynopsis == null) {
					...
					cmdPrint.append(firstSynopsis.length() < INDENT ? firstSynopsis
							: cmds[0]);
				}
				_henplus.msg().print(cmdPrint.toString());
				for (int i = cmdPrint.length(); i < INDENT; ++i) {
					_henplus.msg().print(" ");
				}
				_henplus.msg().print(": ");
				_henplus.msg().println(description);
			}
			...
			CommandDispatcher disp = _henplus.getDispatcher();
			String cmdString = disp.getCommandNameFrom(param);
			Command c = disp.getCommandFrom(param);
			if (c == null) {
				_henplus.msg().println("Help: unknown command '" + param + "'");
				return EXEC_FAILED;
			}
	...
	private void printDescription(String cmdStr, Command c) {
		...
		if (desc != null) {
			...
			_henplus.msg().println("DESCRIPTION");
			_henplus.msg().attributeReset();
			_henplus.msg().println(desc);
			if (c.requiresValidSession(cmdStr)) {
				_henplus.msg().println("\tRequires valid session.");
			}
		}
		if (desc == null && synopsis == null) {
			_henplus.msg().println("no detailed help for '" + cmdStr + "'");
		}
	}
