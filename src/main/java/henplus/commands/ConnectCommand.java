...
//	private DataSourceFactory dsFactory;
	...
	public ConnectCommand(HenPlus henplus, SessionManager sessionManager, BundleContext context) {
//		this.dsFactory = dsFactory;
	...
	private void connect(String url, String username, String password)
		...
		SQLSession session;
		session = new SQLSession(url, username, password,_henplus,context);
	...
	public void registerOptions(Options r) {
		...
		r.addOption(option);
		Option option2 = new Option("U", "username", true,
				"Username to connect with");
		option2.setArgName("username");
		r.addOption(option2);
		Option option3 = new Option("P", "password", true,
...
//			String password = (String) st.nextElement();
			...
			try {
				session = new SQLSession(url, null, null,_henplus,this.context);				
				_knownUrls.put(url, url);				
				if (alias != null) {
					_knownUrls.put(alias, url);
				}
				...
				_henplus.msg().println(e.toString());
				return EXEC_FAILED;
			}
		}
			...
			if (session == null) {
				_henplus.msg().println("'" + sessionName + "': no such session");
				return EXEC_FAILED;
			}
			 ...
			 */
			int renamed = _sessionManager.renameSession(currentSessionName,
					sessionName);
			...
			_henplus.msg().println("session closed.");
	...
	private void showSessions() {
		_henplus.msg().println("current session is marked with '*'");
		for (int i = 0; i < SESS_META.length; ++i) {
			SESS_META[i].resetWidth();
		}
		TableRenderer table = new TableRenderer(SESS_META, _henplus.out());
	...
	public String getSynopsis(String cmd) {
		if ("connect".equals(cmd)) {
			return cmd + " <jdbc-url> <username> <password> [session-name]";
		} else if ("switch".equals(cmd)) {
