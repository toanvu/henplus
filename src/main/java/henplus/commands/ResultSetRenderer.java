...
public class ResultSetRenderer implements Interruptable {
	...
	public ResultSetRenderer(ResultSet rset, String columnDelimiter,
			boolean enableHeader, boolean enableFooter, int limit,
			OutputDevice out, int[] show, HenPlus henplus) throws SQLException {
	...
	public ResultSetRenderer(ResultSet rset, String columnDelimiter,
			boolean enableHeader, boolean enableFooter, int limit,
			OutputDevice out, HenPlus henplus) throws SQLException {
	...
	private String readClob(Clob c) throws SQLException {
		...
		try {
			...
			while (restLimit > 0
					&& (r = in.read(buf, 0,
							(int) Math.min(buf.length, restLimit))) > 0) {
