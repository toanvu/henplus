...
public class SQLCommand extends AbstractCommand {
	...
	private final static String[] TABLE_COMPLETER_KEYWORD = { "FROM", "INTO",
	...
	protected SQLCommand(ListUserObjectsCommand tc, HenPlus henplus) {
		this._henplus = henplus;
		_columnDelimiter = "|";
		_rowLimit = 2000;
		tableCompleter = tc;
	}
...
//		new Thread(_statementCanceller).start();
        Thread cancelThread = new Thread(_statementCanceller);
...
//		new Thread(_longRunningDisplay).start();
        Thread runningDisplay = new Thread(_longRunningDisplay);
        runningDisplay.setDaemon(true);
	...
	private final class CurrentStatementCancelTarget implements
		...
		public void cancelRunningStatement() {
			try {
				_henplus.msg().println("cancel statement...");
				_henplus.msg().flush();
				CancelWriter info = new CancelWriter(_henplus.msg());
	...
	public int execute(SQLSession session, String cmd, String param) {
		...
		try {			
			if (command.startsWith("commit")) {
				...
				if (!_running) {
					_henplus.msg().println("cancelled");
					return SUCCESS;
				}
				...
				if (hasResultSet) {
					...
					renderer = new ResultSetRenderer(rset,
							...
							isShowFooter(), getRowLimit(), _henplus.out(),_henplus);
				...
				if (lapTime > 0) {
					...
					if (session.printMessages()) {
						TimeRenderer.printTime(lapTime, _henplus.msg());
					}
					session.print("; total: ");
				}
				if (session.printMessages()) {
		 ...
		 */
			...
			if (msg != null) {
				...
				_henplus.msg().println("FAILURE: " + msg.trim());
			}
			if (verbose)
				e.printStackTrace();
			return EXEC_FAILED;
