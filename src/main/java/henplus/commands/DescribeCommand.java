...
public class DescribeCommand extends AbstractCommand implements Interruptable {
	...
	public DescribeCommand(ListUserObjectsCommand tc, HenPlus henplus) {
		tableCompleter = tc;
		_henplus = henplus;
	}
	...
	public int execute(SQLSession session, String cmd, String param) {
		...
		while (st.hasMoreTokens()) {
			...
			if (more_options && tabName.startsWith("-")) {
				...
				if (correctName) {
					String alternative = tableCompleter
				...
				try {
					...
					DatabaseMetaData meta = session.getConnection()
							.getMetaData();
					...
					if (rset != null) {
						while (!interrupted && rset.next()) {
							...
							fktable = StringAppender.getInstance()
									.append(fktable).append("(")
									.append(fkcolumn).append(")").toString();
							String desc = (String) pks.get(col);
							desc = (desc == null) ? StringAppender
									.start(" <- ").append(fktable).toString()
									: StringAppender.start(desc)
											.append("\n <- ").append(fktable)
											.toString();
							anyLeftArrow = true;
							pks.put(col, desc);
					...
					_henplus.out().println(
							("VIEW".equals(tableType) ? "View: " : "Table: ")
									+ tabName);
					if (description != null) {
						_henplus.out().println(description);
					}

					if (catalog != null) {
						_henplus.msg().println("catalog: " + catalog);
					}
					if (anyLeftArrow) {
						_henplus.msg().println(" '<-' : referenced by");
					}
					if (anyRightArrow) {
						_henplus.msg().println(" '->' : referencing");
					}
					...
					if (verbose)
						e.printStackTrace();
					String ex = (e.getMessage() != null) ? e.getMessage()
							.trim() : e.toString();
							_henplus.msg().println(
	 ...
	 */
	private void showIndexInformation(String tabName, String schema,
			DatabaseMetaData meta) throws SQLException {
		ResultSet rset;
		_henplus.out().println("index information:");
		...
		if (rset != null)
			while (!interrupted && rset.next()) {
				...
				_henplus.out().print("\t");
				if (!nonUnique)
					_henplus.out().print("unique ");
				_henplus.out().print("index " + idxName);
				String colName = rset.getString(9);
				// work around postgres-JDBC-driver bug:
				if (colName != null && colName.length() > 0) {
					_henplus.out().print(" on " + colName);
				}
				_henplus.out().println();
			}
		rset.close();
		if (!anyIndex) {
			_henplus.out().println("\t<none>");
		}
	}
	...
	public Iterator complete(CommandDispatcher disp, String partialCommand,
		...
		return tableCompleter.completeTableName(_henplus
