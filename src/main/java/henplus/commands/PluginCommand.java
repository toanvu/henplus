...
public final class PluginCommand extends AbstractCommand {
            ...
            public void readConfiguration(InputStream inStream) throws Exception {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    try {
                        ...
                        _henplus.msg().println("couldn't load plugin '" + line + "' "
                                              + e.getMessage());
                    }
	...
	if ("list-plugins".equals(cmd)) {
	    ...
	    _henplus.msg().println("loaded plugins are marked with '*'");
		...
		_henplus.msg().println("plugin '" + pluginClass 
				   + "' already loaded");
		return EXEC_FAILED;
	    }
	    Command plugin = null;
		...
		_henplus.msg().println("couldn't load plugin: " + e.getMessage());
		return EXEC_FAILED;
	    }
	    if (plugin != null) {
		_plugins.put(pluginClass, plugin);
		String[] cmds = plugin.getCommandList();
		_henplus.out().print("adding commands: ");
		for (int i=0; i < cmds.length; ++i) {
		    if (i!=0) _henplus.out().print(", ");
		    _henplus.out().print(cmds[i]);
		}
	    ...
	    String pluginClass = (String) st.nextElement();
	    if (!_plugins.containsKey(pluginClass)) {
	    	_henplus.msg().println("unknown plugin '" + pluginClass + "'");
		return EXEC_FAILED;
	    }
	    else {
