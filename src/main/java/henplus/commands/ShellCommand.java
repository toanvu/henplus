 ...
 */
public class ShellCommand extends AbstractCommand implements Interruptable {
	Thread _myThread;

	private HenPlus henplus;
	
	public ShellCommand(HenPlus henplus){
	...
	public int execute(SQLSession session, String cmd, String param) {
		...
		try {
			try {
				...
				ioHandler = new IOHandler(p,henplus);
			} catch (IOException e) {
				ioHandler.stop();
				return EXEC_FAILED;
			}
...
//		private final Thread stdinThread;
		...
		public IOHandler(Process p, HenPlus henplus) throws IOException {
			this.process = p;
			this.henplus = henplus;
...
//			stdinThread = new Thread(new CopyWorker(System.in, henplus.out()));
