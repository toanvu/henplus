...
public class LongRunningTimeDisplay implements Runnable {
    ...
    public LongRunningTimeDisplay(String msg, long showAfter, HenPlus henplus) {
        _startTimeDisplayAfter = showAfter;
        _message = msg;
        _running = true;
        _armed = false;
        _henplus = henplus;
        _timeDisplay = new CancelWriter(_henplus.msg());
    }
    ...
    public synchronized void run() {
        try {
            for (;;) {
                ...
                if (!_running) return;
                long startDisplayAt = _lastArmTime + _startTimeDisplayAfter;
